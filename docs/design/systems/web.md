# web 端设计系统(v2 · 高级感 + 科技感)

> 文档层级:端级契约(设计系统是 HTML 原型的唯一真相源)
> 适用范围:scoop-gui 项目的所有 web 端页面原型与生产页面
> 上游依赖:`docs/prd/modules/scoop-gui.md`、`docs/architecture/api/scoop-gui.md`、`ARCHITECTURE.md`
> 修订记录:
...
本文件是端级契约,所有 web 端 HTML 原型与生产页面必须严格遵循。
已审批但基于 v1 产出的 10 个 HTML 原型在 v2 批准后**必须重做**。
