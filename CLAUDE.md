...
- `release.yml` 在每次 push 到 `master` 时(纯 `docs/**` / `**/*.md` / `.claude/**` 改动经 `paths-ignore` 跳过,不触发构建与版本递增;混合代码改动仍触发)跑完整 `npm run tauri build`,把 NSIS 安装包发布成一个滚动的 `latest` 预发布版本:**先 `gh release delete latest --cleanup-tag` 再 `gh release create`**,没有用 `tauri-apps/tauri-action` 官方示例的 `tagName: app-v__VERSION__` 版本化发布模式——那种模式假定每次发布对应一个新版本号,不适合"每次提交都产出一个可下载最新构建"的持续构建场景;delete+recreate 避免了资产文件名冲突或旧 asset 残留的问题。以后如果要做真正的正式版本发布(打 tag 触发),应该另开一个 workflow,不要复用 `latest` 这个滚动 tag。
